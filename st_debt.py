...
for i in range(num_debts):
    ...
    if loan_type == "Fixed":
        ...
        remaining_interest = calculate_total_interest(balance, rate, remaining_term)
        ...
        st.write(f"Remaining Term {name}: {remaining_term:,.2f}")
        st.write(f"Periodic Rate : {periodic_rate}")
...
if mortgage_balance > 0 and mortgage_amortization > 0:
    mortgage_payment = calculate_monthly_payment(mortgage_balance, mortgage_rate, mortgage_amortization)
    remaining_mortgage_interest = calculate_total_interest(mortgage_balance, mortgage_rate, mortgage_amortization)
...
if selected_debts and new_amortization > 0:
    new_balance = mortgage_balance + sum(debt['balance'] for debt in debts if debt['name'] in selected_debts) + fees
    consolidated_monthly_payment = calculate_monthly_payment(new_balance, new_rate, new_amortization)
...
if st.button("Compare Scenarios"):
    ...
    st.write(f"Remaining Total Interest to be Paid: ${total_current_interest:,.2f}")  # Includes mortgage interest
